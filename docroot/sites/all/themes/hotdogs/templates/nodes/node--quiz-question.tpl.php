...
// For some reason, array indexing is not consistent so we'll standardize before using
$correct_answer = array();
if( isset($node->field_correct_answer[LANGUAGE_NONE][0]) ) :
  $correct_answer = $node->field_correct_answer[LANGUAGE_NONE];
else :
  $correct_answer = $node->field_correct_answer;
endif;

$incorrect_answer = array();
if( isset($node->field_incorrect_answers[LANGUAGE_NONE][0]) ) :
  $incorrect_answer = $node->field_incorrect_answers[LANGUAGE_NONE];
else :
  $incorrect_answer = $node->field_incorrect_answers;
endif;

// Build an array of possible answers
$possible_answers = array();
$possible_answers[] = array(
  'answer' => $correct_answer[0]['value'],
  'type' => 'correct'
);
foreach( $incorrect_answer as $one_answer ) :
  $possible_answers[] = array(
    'answer' => $one_answer['value'],
    'type' => 'incorrect'
  );
endforeach;
